...
const websocketMiddleware: Middleware = (store) => (next) => (action) => {
  ...
  if (connectWebSocket.match(action)) {
    ...
    if (state.chat.isConnecting || state.chat.isConnected) {
      return result;
    }
    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          switch (message.type) {
            ...
            case "new_message": {
              const newMsg = message as Message;
      ...
      ws.onclose = (event) => {
        ...
        clearTimeout(connectionTimeout);
        store.dispatch(
          setWebSocketConnection({ ws: null, isConnected: false, isConnecting: false })
        );
      ...
      store.dispatch(setWebSocketConnection({ ws: null, isConnected: false, isConnecting: false }));
    }
  }
  ...
  if (sendMessage.match(action)) {
    ...
    const pageType = state.chat.chats[chatId]?.page_type || null;
 ...
 //Handle Typing ON
  if (sendIsTypingOn.match(action)) {
  const state = store.getState();
  const { ws, isConnected } = state.chat;

  if (ws && isConnected && ws.readyState === WebSocket.OPEN) {
    const { chatId } = action.payload;
    ws.send(JSON.stringify({ type: "typing_on", chat_id: chatId}));
  }
}

// Handle typing OFF
if (sendIsTypingOff.match(action)) {
  const state = store.getState();
  const { ws, isConnected } = state.chat;

  if (ws && isConnected && ws.readyState === WebSocket.OPEN) {
    const { chatId } = action.payload;
    ws.send(JSON.stringify({ type: "typing_off", chat_id: chatId}));
  }
}
